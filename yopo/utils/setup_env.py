...
def register_all_modules(init_default_scope: bool = True) -> None:
    """Register all modules in yopo into the registries.

    Args:
        init_default_scope (bool): Whether initialize the yopo default scope.
            When `init_default_scope=True`, the global default scope will be
            set to `yopo`, and all registries will build modules from yopo's
            registry node. To understand more about the registry, please refer
            to https://github.com/open-mmlab/mmengine/blob/main/docs/en/tutorials/registry.md
            Defaults to True.
    """  # noqa
    import yopo.datasets  # noqa: F401,F403
    import yopo.engine  # noqa: F401,F403
    import yopo.evaluation  # noqa: F401,F403
    import yopo.models  # noqa: F401,F403
    import yopo.visualization  # noqa: F401,F403

    if init_default_scope:
        never_created = DefaultScope.get_current_instance() is None \
                        or not DefaultScope.check_instance_created('yopo')
        if never_created:
            DefaultScope.get_instance('yopo', scope_name='yopo')
            return
        current_scope = DefaultScope.get_current_instance()
        if current_scope.scope_name != 'yopo':
            warnings.warn('The current default scope '
                          f'"{current_scope.scope_name}" is not "yopo", '
                          '`register_all_modules` will force the current'
                          'default scope to be "yopo". If this is not '
                          'expected, please set `init_default_scope=False`.')
            # avoid name conflict
            new_instance_name = f'yopo-{datetime.datetime.now()}'
            DefaultScope.get_instance(new_instance_name, scope_name='yopo')
