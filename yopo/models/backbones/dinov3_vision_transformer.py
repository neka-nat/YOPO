...
class DinoVisionTransformer(BaseModule):
    def __init__(
        ...
        **ignored_kwargs,
        ...
        if arch not in vit_sizes.keys():
            raise ValueError(f"arch should be one of {list(vit_sizes.keys())}")

        # Use arch configuration, but allow override with explicit parameters
        arch_config = vit_sizes[arch]
        embed_dim = embed_dim if embed_dim is not None else arch_config["embed_dim"]
        depth = depth if depth is not None else arch_config["depth"]
        num_heads = num_heads if num_heads is not None else arch_config["num_heads"]
        ffn_ratio = ffn_ratio if ffn_ratio is not None else arch_config["ffn_ratio"]
    ...
    def _freeze_stages(self):
        ...
        # Freeze norm layers if all blocks are frozen
