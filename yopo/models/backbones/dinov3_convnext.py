...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0.0, layer_scale_init_value=1e-6, with_cp: bool = False):
        super().__init__()
...
class DinoConvNeXt(BaseModule):
    ...
    def __init__(
        ...
        if arch in self.convnext_sizes:
            arch_args = self.convnext_sizes[arch]
            depths = arch_args["depths"]
            dims = arch_args["dims"]
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[
                    Block(dim=dims[i], drop_path=dp_rates[cur + j], layer_scale_init_value=layer_scale_init_value, with_cp=with_cp)
        ...
        for i in self.out_indices:
            norm_layer = LayerNorm2d(dims[i], eps=1e-6)
    ...
    def init_weights(self):
        ...
        if self.init_cfg is None:
            ...
            # remove parameter for linear classifier
            pop_list = ['norm.weight', 'norm.bias', 'norms.3.weight', 'norms.3.bias']
            for key in pop_list:
                if key in ckpt:
                    ckpt.pop(key)
    ...
    def forward(self, x: Tensor, masks: Optional[Tensor] = None, is_training=False
    ) -> Union[Dict[str, Tensor], Tensor]:
        xs = self.get_intermediate_layers(x, n=len(self.out_indices), reshape=True)
        return xs

    def forward_original(self, *args, is_training=False, **kwargs):
    ...
    def _get_intermediate_layers(self, x, n=1, return_class_token=False):
        h, w = x.shape[-2:]
    ...
    def get_intermediate_layers(
        ...
        outputs = self._get_intermediate_layers(x, n, return_class_token)

        if norm:
            nchw_shapes = [out[-1].shape for out in outputs]
            outputs = [
                    patches.flatten(-2, -1).permute(0, 2, 1)  # N x HW x C
                for cls_token, patches in outputs
            ]
            if reshape:
                outputs = [
                    patches.permute(0, 2, 1).reshape(*nchw).contiguous()
                    for patches, nchw in zip(outputs, nchw_shapes)
                ]
        elif not reshape:
            # force B x N x C format for patch tokens
            outputs = [patches.flatten(-2, -1).permute(0, 2, 1) for patches in outputs]
        # apply layernorm to each output (B, N, C)
