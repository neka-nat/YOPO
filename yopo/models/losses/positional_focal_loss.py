...
def py_sigmoid_focal_loss(pred,
    ...
    # Unified soft / hard (one-hot) target focal loss:
    # p_t is the prob assigned to the "target" (soft when target in [0,1])
    p_t = pred_sigmoid * target + (1 - pred_sigmoid) * (1 - target)
    # Standard focal modulating factor
    modulating_factor = (1 - p_t).pow(gamma)
    # Alpha balancing for soft targets
    alpha_weight = alpha * target + (1 - alpha) * (1 - target)
    # Stable BCE with logits for soft labels
    bce = F.binary_cross_entropy_with_logits(pred, target, reduction='none')
    loss = bce * alpha_weight * modulating_factor
...
class PositionalFocalLoss(nn.Module):
    ...
    def __init__(self,
        ...
        Args:
            ...
            activated (bool, optional): Whether the input is activated.
                ...
                Defaults to False.
        """
        super(PositionalFocalLoss, self).__init__()
