...
class Reconstructor(nn.Module):
    def __init__(self, points_per_keypoint, ndim):
        super().__init__()
        self.pts_per_kpt = points_per_keypoint
        self.ndim = ndim
...
class GeometricAwareFeatureAggregator(nn.Module):
    def __init__(self, block_num, K, d_model):
        super().__init__()
...
class NOCS_Predictor(nn.Module):
    def __init__(self, num_classes, num_bins, attn_cfg):
        super().__init__()
        self.num_bins = num_bins
        self.num_classes = num_classes
        bin_length = 1 / self.num_bins
        half_bin_length = bin_length / 2
        self.bins_center = torch.linspace(start=-0.5, end=0.5-bin_length, steps=self.num_bins).view(self.num_bins, 1).cuda()
        self.bins_center = self.bins_center + half_bin_length  # (bins_num, 1)
        ...
        self.atten_mlp = nn.Sequential(
            ...
            nn.Conv1d(512, 512, 1),
            nn.ReLU(),
            nn.Conv1d(512, self.num_classes*3*self.num_bins, 1),
    ...
    def forward(self, kpt_feature, index):
        ...
        attn = attn.view(b*self.num_classes, 3*self.num_bins, kpt_num).contiguous()
        attn = torch.index_select(attn, 0, index)
        attn = attn.view(b, 3, self.num_bins, kpt_num).contiguous()
...
class SelfAttnLayer(nn.Module):
    def __init__(self, block_num, d_model=256, num_heads=4, dim_ffn=256):
        super().__init__()
        self.block_num = block_num
        self.d_model = d_model
        ...
        self.attn_blocks = nn.ModuleList()
        for i in range(self.block_num):
            self.attn_blocks.append(SelfAttnBlock(d_model=self.d_model, num_heads=self.num_heads, dim_ffn=self.dim_ffn, dropout=0.0, dropout_attn=None))
...
class AttnLayer(nn.Module):
    def __init__(self, block_num, d_model=256, num_heads=4, dim_ffn=256):
        ...
        self.attn_blocks = nn.ModuleList()
        for i in range(self.block_num):
            self.attn_blocks.append(AttnBlock(d_model=self.d_model, num_heads=self.num_heads, dim_ffn=self.dim_ffn, dropout=0.0, dropout_attn=None))    


class InstanceAdaptiveKeypointDetector(nn.Module):
    def __init__(self, num_keypoints, query_dim,  attn_cfg):
        super().__init__()
        self.num_keypoints = num_keypoints
        self.query_dim = query_dim
        # initialize shared kpt_query for all categories
        self.kpt_query = nn.Parameter(torch.empty(self.num_keypoints, self.query_dim)) # (kpt_num, query_dim)
        nn.init.xavier_normal_(self.kpt_query)
        
        # build attention layer
        self.attn_layer = AttnLayer(**attn_cfg)
        
    def forward(self, ptsrgb_feature):
        """_summary_

        Args:
            ptsrgb_local (_type_): (b, 2c, n)
            cls:                (b, )
        """
        b, c, n = ptsrgb_feature.shape
        
        batch_kpt_query = self.kpt_query.unsqueeze(0).repeat(b, 1, 1)
        # (b, kpt_num, 2c)  (b, kpt_num, n)
        batch_kpt_query, attn = self.attn_layer(batch_kpt_query, ptsrgb_feature) 
        
        # cos similarity <a, b> / |a|*|b|
        norm1 = torch.norm(batch_kpt_query, p=2, dim=2, keepdim=True) 
        norm2 = torch.norm(ptsrgb_feature, p=2, dim=1, keepdim=True) 
        heatmap = torch.bmm(batch_kpt_query, ptsrgb_feature) / (norm1 * norm2 + 1e-7)
