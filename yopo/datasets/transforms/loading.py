...
from yopo.datasets.pose_estimation.nocs_utils import fill_missing_depth

from yopo.registry import TRANSFORMS
from yopo.structures.bbox import get_box_type
from yopo.structures.bbox.box_type import autocast_box_type
from yopo.structures.mask import BitmapMasks, PolygonMasks
...
class LoadMultiChannelImageFromFiles(BaseTransform):
    ...
    Args:
        ...
        file_client_args (dict): Arguments to instantiate the
            corresponding backend in yopo <= 3.0.0rc6. Defaults to None.
        backend_args (dict, optional): Arguments to instantiate the
            corresponding backend in yopo >= 3.0.0rc7. Defaults to None.
    """
    ...
    def transform(self, results: dict) -> dict:
        ...
        Args:
            results (dict): Result dict from :obj:`yopo.CustomDataset`.
...
@TRANSFORMS.register_module()
class LoadPoseAnnotations(LoadAnnotations):
    def __init__(self,
                 with_bbox: bool = True,
                 with_pose: bool = True,
        ...
        super(LoadPoseAnnotations, self).__init__(
            with_bbox=with_bbox,
    ...
    def _load_pose(self, results: dict) -> None:
        ...
        for instance in instances:
            rotations.append(instance['rotation'])
            centers_2d.append(instance['center_2d'])
            zs.append(instance['z'])
            Ts.append(instance['T'])

        n = len(instances)

        results['rotation'] = np.array(rotations, dtype=np.float32).reshape(n, -1)
        results['center_2d'] = np.array(centers_2d, dtype=np.float32).reshape(n, -1)
        results['z'] = np.array(zs, dtype=np.float32).reshape(n, -1)
        results['T'] = np.array(Ts, dtype=np.float32)
...
class InferencerLoader(BaseTransform):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.from_ndarray = TRANSFORMS.build(
            dict(type='yopo.LoadImageFromNDArray', **kwargs))
...
@TRANSFORMS.register_module()
class Load9DPoseAnnotations(LoadPoseAnnotations):
    ...
    def _load_pose(self, results: dict) -> None:
        ...
        sizes = []
        translations = []
        gt_handle_visibilities = []

        Ts = []
        for instance in instances:
            rotations.append(instance['rotation'])
            translations.append(instance['translation'])
            sizes.append(instance['size'])
            Ts.append(instance['T'])

            if 'gt_handle_visibility' in instance:
                gt_handle_visibilities.append(instance['gt_handle_visibility'])
        
        if self.with_centers_2d:
            centers_2d = []
            if self.with_z:
                zs = []
            for instance in instances:
        ...
        results['T'] = np.array(Ts, dtype=np.float32)
        results['rotation'] = np.array(rotations, dtype=np.float32).reshape(n, -1)
        results['size'] = np.array(sizes, dtype=np.float32).reshape(n, -1)
        results['translation'] = np.array(translations, dtype=np.float32).reshape(n, -1)
        if self.with_centers_2d:
            results['center_2d'] = np.array(centers_2d, dtype=np.float32).reshape(n, -1)
            if self.with_z:
                results['z'] = np.array(zs, dtype=np.float32).reshape(n, -1)
...
class LoadDepthImageFromFile(BaseTransform):
    ...
    Args:
        ...
        imdecode_backend (str): The image decoding backend type. The backend
            ...
            See :func:`mmcv.imfrombytes` for details.
            Defaults to 'cv2'.
        img_scale (list[int, int]): The scale of the image to be loaded.
            (width, height).
            Defaults to [640, 480].
        norm_scale (float): The scale to normalize the depth image.
            Defaults to 1000.0.
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        assert depth is not None, f'failed to load image: {filename}'
        ...
        depth = fill_missing_depth(depth16, self.norm_scale, 1)
        depth = depth / self.norm_scale
...
@TRANSFORMS.register_module()
class ConvertDepthToPointCloud(BaseTransform):
    ...
    def __init__(self,
        ...
        self.concatenate_to_rgb = concatenate_to_rgb

        self.xmap = np.array([[i for i in range(width)] for j in range(height)])
        self.ymap = np.array([[j for i in range(width)] for j in range(height)])
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        # depth = results['depth']
        depth = results.pop('depth', None)
        if depth is None:
            raise ValueError('Depth image is not found in results.')
        intrinsic = results['intrinsic']
        # pts
        cam_fx, cam_fy, cam_cx, cam_cy = intrinsic

        z_coords = depth.copy()
        x_coords = (self.xmap - cam_cx) * z_coords / cam_fx
        y_cxoords = (self.ymap - cam_cy) * z_coords / cam_fy
        pts_3d_all = np.stack([x_coords, y_cxoords, z_coords], axis=-1)  # H*W*3

        before_shape = results['img'].shape
        if self.concatenate_to_rgb:
            # Concatenate the point cloud to the image
            img = results['img']
            # assume img is H*W*3
            pts_3d_all = np.concatenate([img, pts_3d_all], axis=-1)  # H*W*6
            results['img'] = pts_3d_all
        else:
            # Just keep the point cloud
            results['points'] = pts_3d_all
