...
def convert(src: str, dst: str, prefix: str = 'd2_model') -> None:
    """Convert Detectron2 checkpoint to YOPO style.

    Args:
        src (str): The Detectron2 checkpoint path, should endswith `pkl`.
        dst (str): The YOPO checkpoint path.
        prefix (str): The prefix of YOPO model, defaults to 'd2_model'.
    """
    # load arch_settings
    ...
    yopo_model = dict(state_dict=dst_state_dict, meta=dict())
    save_checkpoint(yopo_model, dst)
    print(f'Convert Detectron2 model {src} to YOPO model {dst}')


def main():
    parser = argparse.ArgumentParser(
        description='Convert Detectron2 checkpoint to YOPO style')
    parser.add_argument('src', help='Detectron2 model path')
    parser.add_argument('dst', help='YOPO model save path')
    parser.add_argument(
